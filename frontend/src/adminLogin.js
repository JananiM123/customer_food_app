...
const AdminLogin = ({setIsLogIn}) => {
  ...
  const handleSubmit = (event) => {
    ...
    const foundUser = isUser.email === 'admin' && isUser.password === 'admin';

    if (isUser.email === '' || isUser.password === '') {
      obj = { ...obj, error: 'Please enter both admin email & password ' }
    }
    else if (foundUser) {
      navigate(`/food`);
      setIsLogIn(true);
      ...
      obj = { ...obj, error: 'Please check admin email & password' }
    }
    setErrorMsg(obj)
  };
